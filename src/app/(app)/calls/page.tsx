        ...
        <div className="relative w-full">
          <div className="mb-9 flex flex-col gap-6">
            <div>
              ...
              <h1 className="text-display-hero leading-tight tracking-tight text-on-surface">
                {greeting(firstName)}
              </h1>
            </div>
          </div>
          ...
          <div className="mb-9 grid grid-cols-1 gap-6 lg:grid-cols-5">
            <div className="relative flex min-h-[280px] flex-col justify-between overflow-hidden rounded-3xl bg-gradient-to-br from-[#FFF8E7] to-[#FFF1CC] p-9 shadow-sm transition-all duration-300 hover:shadow-md lg:col-span-3">
              ...
              <div className="relative flex items-start justify-between">
                ...
                <span className="rounded-full bg-[#FFE7A8] px-3 py-0.5 text-label-sm text-on-secondary-fixed">
                  Instant mic
                </span>
              </div>
              <div className="relative my-4">
                <h2 className="text-headline-lg mb-1.5 text-on-secondary-fixed">
                  Capture a meeting
                </h2>
                <p className="text-body-md leading-relaxed text-on-secondary-fixed-variant">
                  Live AI voice call, recorded &amp; transcribed
                </p>
              </div>
              <Link
                href="/room"
            ...
            <div className="flex flex-col gap-6 lg:col-span-2">
              <div className="relative flex min-h-[128px] flex-1 flex-col justify-between overflow-hidden rounded-3xl bg-gradient-to-br from-[#F5EDFF] to-[#EBDDFF] p-6 shadow-sm transition-all duration-300 hover:shadow-md">
                ...
                <div className="relative">
                  <h2 className="text-headline-sm text-on-tertiary-fixed">
                    Ask your calls
                  </h2>
                  <p className="text-body-sm text-on-tertiary-fixed-variant">
                    Chat across everything discussed
                  </p>
                </div>
              </div>
              ...
              <div className="relative flex min-h-[128px] flex-1 flex-col justify-between overflow-hidden rounded-3xl bg-[#1C1924] p-6 shadow-md transition-all duration-300 hover:shadow-xl">
                ...
                <div className="relative">
                  <h2 className="text-headline-sm text-white">
                    Semantic search
                  </h2>
                  <p className="text-body-sm text-[#CAC4D3]">
          ...
          <div className="grid grid-cols-1 items-start gap-9 lg:grid-cols-12">
            <div
              id="recent-meetings"
              className="flex flex-col gap-4 scroll-mt-24 lg:col-span-7"
            >
              <h2 className="text-headline-md px-1 text-on-surface">
                Recent meetings
              </h2>
            ...
            <div className="flex flex-col gap-4 lg:col-span-5">
              ...
              <div className="flex flex-col gap-3">
                {recentDigests.length === 0 ? (
                  ...
                  recentDigests.map((m) => {
                    const summary =
                      m.summaries.find((s) => s.template === "general") ??
                      m.summaries[0];
                    ...
                    return (
                      <div
                        key={m.id}
                        className="flex flex-col gap-4 rounded-3xl bg-surface-container-lowest/80 p-6 shadow-sm backdrop-blur-md transition-all hover:shadow-md"
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-center gap-3">
                            <Avatar
                              name={m.participants[0]?.name ?? m.title}
                              size={36}
                            />
                            <div>
                          ...
                          {openCount > 0 ? (
                            <span className="rounded-full bg-[#F4EEFF] px-2.5 py-0.5 text-label-sm text-primary">
                              {openCount} action items
                            </span>
                          ) : m.status === "ready" ? (
                            <span className="rounded-full bg-[#FFF8E7] px-2.5 py-0.5 text-label-sm text-on-secondary-fixed">
                              Summarized
                            </span>
                          ) : null}
                        </div>
                        <p className="line-clamp-2 text-body-md leading-relaxed text-on-surface-variant">
                          {summary?.content?.purpose ||
                            "Summary generates automatically once the transcript is processed."}
                        </p>
                        <div className="flex items-center justify-between pt-1">
                          ...
                          <Link
                            href={`/m/${m.id}`}
                            className="inline-flex items-center gap-1 text-label-sm text-primary hover:text-on-primary-fixed-variant"
                          >
                            View Brief
                            <ArrowRight className="h-4 w-4" />
                          </Link>
  ...
  return (
    <div className="flex flex-col items-center gap-4 rounded-3xl bg-surface-container-lowest/80 px-6 py-16 text-center shadow-sm backdrop-blur-md">
      ...
      <p className="text-body-md text-on-surface-variant">
        Start a test meeting to see a real transcript, summary, and clips.
      </p>
      <Link
        href="/room"
        className="brand-gradient rounded-full px-5 py-2.5 text-label-lg text-white shadow-[0_8px_20px_-4px_rgba(168,85,247,0.35)]"
      >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <div
          className={cn(
            "flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-xs",
            tone.iconColor,
          )}
        >
